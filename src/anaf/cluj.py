cluj = {
    "Directia Generala Regionala a Finantelor Publice Cluj Napoca":
        {
    "Bihor": {
        "Administratia Judeteana a Finantelor Publice BIHOR": [
            "Biharia", "Bors", "Ceica", "Cetariu", "Ciuhoi", "Copacel", "Diosig", 
            "Dragesti", "Gepiu", "Girisul de Cris", "Hidiselu de Sus", "Ineu", "Lazereni", 
            "Nojorid", "Oradea", "Osorhei", "Paleu", "Rosiori", "Sanmartin", "Santandrei", 
            "Sarbi", "Sacadat", "Salard", "Tamaseu", "Toboliu", "Virciorog"
        ],
        "Unitatea Fiscala Oraseneasca Alesd": [
            "Alesd", "Astileu", "Auseu", "Borod", "Bratca", "Brusturi", "Bulz", 
            "Lugasu de Jos", "Magesti", "Shinteu", "Shuncuius", "Tileagd", "Tetchea", "Vadu Crisului"
        ],
        "Unitatea Fiscala Oraseneasca Beius": [
            "Beius", "Budureasa", "Buntesti", "Campani", "Capina", "Carpinet", "Cristioru de Jos", 
            "Curatele", "Dobresti", "Draganesti", "Finis", "Holod", "Lazuri de Beius", "Lunca", 
            "Nucet", "Pietroasa", "Pocola", "Pomezeu", "Rabagani", "Remetea", "Rieni", "Rosia", 
            "Sambata", "Soimi", "Shtei", "Tarcaia", "Uileacu de Beius", "Vascau"
        ],
        "Unitatea Fiscala Oraseneasca Marghita": [
            "Abram", "Abramut", "Balc", "Boianu Mare", "Buduslau", "Cherechiu", "Chislaz", "Curtuiseni", 
            "Derna", "Marghita", "Popesti", "Sacuieni", "Salacea", "Spinus", "Suplacul de Barca", "Simian", 
            "Tarcea", "Tauteu", "Valea lui Mihai", "Viisoara"
        ],
        "Unitatea Fiscala Oraseneasca Salonta": [
            "Avram Iancu", "Batar", "Cefa", "Ciumeghiu", "Cociuba Mare", "Husasau de Tinca", "Madaras", 
            "Olcea", "Salonta", "Sannicolau Roman", "Tinca", "Tulca"
        ]
    },
    "Bistrita Nasaud": {
        "Administratia Judeteana a Finantelor Publice BISTRITA NASAUD": [
            "Bistrita", "Budacu de Jos", "Cetate", "Dumitrita", "Mariselu", "Monor", "Sieu", "Sieu Magherus", 
            "Sieut", "Bistrita Bargau", "Josenii Bargau", "Livezile", "Prundu Bargau", "Tiha Bargau", 
            "Budesti", "Galatii Bistritei", "Lechinta", "Matei", "Micestii de Campie", "Milas", 
            "Sanmihaiu de Campie", "Silivasu de Campie", "Teaca", "Urmenis"
        ],
        "Unitatea Fiscala Oraseneasca Beclean": [
            "Beclean", "Branistea", "Caianu Mic", "Chiochis", "Chiuza", "Ciceu Giurgesti", "Ciceu Mihaiesti", 
            "Negrilesti", "Nuseni", "Petru Rares", "Spermezeu", "Sieu Odorhei", "Sintereag", "Tarlisua", 
            "Uriu"
        ],
        "Unitatea Fiscala Oraseneasca Nasaud": [
            "Cosbuc", "Dumitra", "Feldru", "Nasaud", "Nimigea", "Parva", "Rebra", "Rebrisoara", "Romuli", 
            "Runcu Salvei", "Salva", "Telciu", "Zagra"
        ],
        "Unitatea Fiscala Oraseneasca Sangeorz Bai": [
            "Maieru", "Rodna", "Sangeorz Bai", "Sant", "Ilva Mica", "Ilva Mare", "Lesu", "Lunca Ilvei", 
            "Magura Ilvei", "Poiana Ilvei"
        ]
    },
    "Cluj": {
        "Administratia Judeteana a Finantelor Publice CLUJ": [
            "Aghiresu", "Apahida", "Aschileu", "Baciu", "Caianu", "Camarasu", "Chinteni", "Ciurila", "Cluj Napoca", 
            "Cojocna", "Feleacu", "Floresti", "Garbo", "Gilaul", "Jucu", "Maguri Racatau", "Mociu", "Panticeu", 
            "Palatca", "Sanpaul", "Savadisla", "Suatu", "Vultureni"
        ],
        "Unitatea Fiscala Municipala Turda": [
            "Aiton", "Baisoara", "Campia Turzii", "Calarasi", "Ceanu Mare", "Frata", "Iara", "Luna", 
            "Mihai Viteazu", "Moldovenesti", "Petrestii de Jos", "Ploscos", "Sandulesti", "Tritenii de Jos", 
            "Turda", "Tureni", "Valea Ierii", "Viisoara"
        ],
        "Unitatea Fiscala Municipala Dej": [
            "Bobilna", "Catcau", "Caseiu", "Chiuiesti", "Cuzdrioara", "Dej", "Jichisu de Jos", "Mica", 
            "Recea Cristur", "Unguras", "Vad"
        ],
        "Unitatea Fiscala Municipala Gherla": [
            "Alunis", "Bontida", "Borsa", "Buza", "Catina", "Cornesti", "Dabaca", "Fizesu Gherlii", "Geaca", 
            "Gherla", "Iclod", "Mintiu Gherlii", "Sanmartin", "Sic", "taga"
        ],
        "Unitatea Fiscala Oraseneasca Huedin": [
            "Belis", "Calatele", "Capusu Mare", "Ciucea", "Huedin", "Izvoru Crisului", "Manastireni", "Margau", 
            "Marisel", "Negreni", "Poieni", "Rasca", "Sancraiu", "Sacuieu"
        ]
    },
    "Maramures": {
        "Administratia Judeteana a Finantelor Publice MARAMURES": [
            "Ardusat", "Arinis", "Asuaju de Sus", "Baia Mare", "Baia Sprie", "Baita de sub Codru", "Basesti", 
            "Bicaz", "Cavnic", "Cicarlau", "Coas", "Coltau", "Copalnic Manastur", "Dumbravita", "Farcasa", "Garda", 
            "Grosi", "Oarta de Jos", "Recea", "Sacalaseni", "Salsig", "Seini", "sisesti", "Tautii Magheraus", 
            "Ulmeni", "Boiu Mare", "Miresu Mare", "Remetea Chioarului", "Satulung", "somcuta Mare", "Valea Chioarului"
        ],
        "Unitatea Fiscala Municipala Sighetu Marmatiei": [
            "Barsana", "Bocicoiu Mare", "Budesti", "Calinesti", "Campulung la Tisa", "Desesti", "Giulesti", 
            "Ocna Sugatag", "Oncesti", "Remeti", "Rona de Jos", "Rona de Sus", "Sarasau", "Sapanta", 
            "Sighetu Marmatiei", "Stramtura", "Vadu Izei"
        ],
        "Unitatea Fiscala Oraseneasca Targu Lapus": [
            "Baiut", "Cernesti", "Coroieni", "Cupsenii", "Grosii Tiblesului", "Lapus", "Suciu de Sus", 
            "Targu Lapus", "Vima Mica"
        ],
        "Unitatea Fiscala Oraseneasca Viseu de Sus": [
            "Bistra", "Bogdan Voda", "Borsa", "Botiza", "Dragomiresti", "Ieud", "Leordina", "Moisei", 
            "Petrova", "Poienile de sub Munte", "Poienile Izei", "Repedea", "Rozavlea", "Ruscova", 
            "Sacel", "Salistea de Sus", "Sieu", "Viseu de Jos", "Viseu de Sus"
        ]
    },
    "Administratia Judeteana a Finantelor Publice SATU MARE": {
        "Administratia Judeteana a Finantelor Publice Satu Mare": [
            "Agris", "Apa", "Ardud", "Barsau", "Batarci", "Beltiug", "Botiz", "Crucisor", "Culciu", "Doba", 
            "Dorolț", "Halmeu", "Homoroade", "Lazuri", "Livada", "Mediesu Aurit", "Micula", "Odoreu", 
            "Paulesti", "Pomi", "Porumbesti", "Satmarel", "Satu Mare", "Socond", "Tarna Mare", "Terebesti", 
            "Turulung", "Valea Vinului", "Vetis", "Viile Satu Mare"
        ],
        "Unitatea Fiscala Municipala Carei": [
            "Berveni", "Carei", "Camin", "Capleni", "Ciumesti", "Craidorolt", "Foieni", "Ianculesti", 
            "Moftin", "Petresti", "Pistol", "Sanislau", "Tiream", "Urziceni"
        ],
        "Unitatea Fiscala Oraseneasca Negresti Oas": [
            "Bixad", "Calinesti - Oas", "Camarzana", "Certeze", "Gherta Mica", "Luna", "Negresti-Oas", 
            "Orasu Nou", "Racsa", "Tarsolt", "Tur", "Turt", "Vama"
        ],
        "Unitatea Fiscala Oraseneasca Tasnad": [
            "Acas", "Andrid", "Blaja", "Bogdand", "Cauas", "Cehal", "Cig", "Hodod", "Pir", "Ratiu", 
            "Santau", "Sarauad", "Sacaseni", "Sauca", "Supur", "Tasnad", "Valea Morii"
        ],
    },
    "Administratia Judeteana a Finantelor Publice SALAJ": {
        "Administratia Judeteana a Finantelor Publice Salaj": [
            "Agrij", "Almasu", "Banisor", "Bocsa", "Buciumi", "Cizer", "Coseiu", "Crasna", "Criseni", 
            "Cuzapac", "Dobrin", "Dragu", "Fildu de Jos", "Hereclean", "Hida", "Horoatu Crasnei", 
            "Mesesenii de Jos", "Mirsid", "Romanasi", "Sanmihaiu Almasului", "Sarmasag", "Samsud", 
            "Treznea", "Zalau", "Zimbor"
        ],
        "Unitatea Fiscala Oraseneasca Simleu Silvaniei": [
            "Bobota", "Carastelec", "Chiesd", "Maeriste", "Pericei", "Simleu Silvaniei", "Varsolt", 
            "Boghish", "Camar", "Halmasd", "Ip", "Marca", "Nusfalau", "Plopis", "Sag", "Valcau de Jos"
        ],
        "Unitatea Fiscala Oraseneasca Jibou": [
            "Babeni", "Balan", "Creaca", "Cristolt", "Galgau", "Garbau", "Ileanda", "Jibou", "Letca", 
            "Lozna", "Napradea", "Poiana Blenchii", "Rus", "Simisna", "Somes Odorhei", "Surduc", "Zalha"
        ],
        "Unitatea Fiscala Oraseneasca Cehu Silvaniei": [
            "Benesat", "Cehu Silvaniei", "Salatig"
        ]
    }
}
}